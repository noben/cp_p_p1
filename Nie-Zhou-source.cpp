    ...
    for (int k=0; k<plaintext.size(); k++)
    {
        ...
        for (; i<ctext_size-t; i++)
        {
            if ((ctext[i]-temp_plain_string[i]) != (ctext[i+t]-temp_plain_string[i+t])
                && (ctext[i]-temp_plain_string[i]) != (26+ctext[i+t]-temp_plain_string[i+t])
                && (26+ctext[i]-temp_plain_string[i]) != (ctext[i+t]-temp_plain_string[i+t]))
            {
                break;
            }
        }
            ...
            for (; i<t; i++)
            {
                ...
                for (int j=0; j<y-1; j++)
                {
                    if ( (ctext[i*y+j] - temp_plain_string[i*y+j]) != (ctext[i*y+j+1] - temp_plain_string[i*y+j+1])
                        && (ctext[i*y+j] - temp_plain_string[i*y+j]) != (26+ctext[i*y+j+1] - temp_plain_string[i*y+j+1])
                        && (26+ctext[i*y+j] - temp_plain_string[i*y+j]) != (ctext[i*y+j+1] - temp_plain_string[i*y+j+1]) )
                    {
                        flag = 0;
                        break;
                    }
            ...
            if (i == t)
            {
                ...
                for (; k<L-t*y; k++)
                {
                    if ( (ctext[k] - temp_plain_string[k]) != (ctext[k+t*y] - temp_plain_string[k+t*y])
                        && (26+ctext[k] - temp_plain_string[k]) != (ctext[k+t*y] - temp_plain_string[k+t*y])
                        && (ctext[k] - temp_plain_string[k]) != (26+ctext[k+t*y] - temp_plain_string[k+t*y]) )
                    {
                        break;
                    }
                }
...
int main(int argc, char *argv[]) {
    ...
    else
    {
        ...
        while ( len_text >> temp_plaintext_line)
        {
            int plaintext_line_size = temp_plaintext_line.size();
            for (int i=0; i<=plaintext_line_size-cipher_string.size(); i++)
            {
                plaintext.push_back(temp_plaintext_line.substr(i, cipher_string.size()));
            }
    ...
    // {
    // 	cout<<"Candidate Plaintext: "<<plaintext[i]<<endl;
    // }
